...
public class TimeTableView extends LinearLayout {
    ...
    //线宽
    protected     int tableLineWidth = 3;
    //数字字体大小
     ...
     */
    protected void flushView(Map<Integer, List<Course>> courseMap) {
        //初始化主布局
     ...
     * @param day        星期
     */
    protected void addDayCourse(ViewGroup pViewGroup, Map<Integer, List<Course>> courseMap, int day) {
        ...
        if (null != courses) {
            for (int i = 0, size = courses.size(); i < size; i++) {
                ...
                if (i == 0) addBlankCell(linearLayout, section - 1);
                    ...
                    addBlankCell(linearLayout, course.getSection() - courses.get(i - 1).getSection() - 1);
                addCourseCell(linearLayout, course);
                if (i == size - 1) addBlankCell(linearLayout, maxSection - section);
            }
        } else {
     ...
     * @param course 课程信息
     */
    protected void addCourseCell(ViewGroup pViewGroup, Course course) {
        addHorizontalTableLine(pViewGroup);
        RoundTextView textView = new RoundTextView(mContext, radius, getColor(colorMap, course.getCourseName()));
        textView.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, cellHeight));
        textView.setTextSize(courseSize);
        textView.setTextColor(Color.WHITE);
        textView.setGravity(Gravity.CENTER);
        textView.setText(String.format(course.getCourseName()));
     ...
     * @param num        空白块数量
     */
    protected void addBlankCell(ViewGroup pViewGroup, int num) {
        for (int i = 0; i < num; i++) {
            addHorizontalTableLine(pViewGroup);
...
//                }
//            });
//            blank.setOnLongClickListener(new OnLongClickListener() {
//                @Override
